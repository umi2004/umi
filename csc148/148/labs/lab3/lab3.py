...
class Player:
    """
    A player of the number game.

    === Attributes ===
    name: str
        Identify the name that user select
    type: str
        Identify the type that user select
    """
    name: str
    ...
    def move(self, current: int, _min: int, _max: int, goal: int)-> int:
...
class RandomPlayer(Player):
    ...
    def move(self, current: int, _min: int, _max: int, goal: int)-> int:
        a = _min
        b = min(goal - current, _max)
        current += random.randint(a, b)
        return current
...
def make_player(generic_name: str) -> Player:
    ...
    type = input(f'Enter a type of player: ')

    if type == 'RandomPlayer':
        return RandomPlayer(name)
    elif type == 'UserPlayer':
